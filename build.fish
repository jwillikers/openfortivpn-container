...
podman run --rm --arch $architecture --volume $mountpoint:/mnt:Z registry.fedoraproject.org/fedora:latest \
    bash -c "dnf -y install dnf-utils; dnf -y config-manager --add-repo https://repo.fortinet.com/repo/6.4/centos/7/os/x86_64/fortinet.repo; dnf -y install --installroot /mnt --releasever 35 glibc-minimal-langpack bash forticlient coreutils --nodocs --setopt install_weak_deps=False"
or exit
